...
def preprocess_row(row, tokenizer, max_span_len=10, filtered_spans=True, aspect_sentiment_only=False):
    ...
    print(f"[INFO] Preprocessing: \"{text}\"")
    ...
    for ann in annotations:
        ...
        if aspect_sentiment_only: opinion_span = find_span(ann['opinion'])
        ...
        if aspect_span and aspect_sentiment_only:
    ...
    for span in spans:
        if span in target_spans and span in opinion_spans:
            print(f"[WARNING] Span {span} is both aspect and opinion in: \"{text}\"")
            mention_labels.append(0)  # or choose how to handle
        elif span in target_spans:
            mention_labels.append(0)
        elif span in opinion_spans:
            mention_labels.append(1)
        else:
    ...
    if aspect_sentiment_only==False:
        triplet_labels = []
        for t in spans:
            for o in spans:
                if t == o: continue
