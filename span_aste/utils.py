...
def generate_candidate_pairs(mentions):
    ...
    for t_span, _, t_embed in targets:
        for o_span, _, o_embed in opinions:
            ...
            # target_opinion_pairs.append((t_embed, o_embed, distance))
...
def compute_loss(
    ...
    # Mention loss setup
    if use_focal:
        alpha_mention = torch.tensor([0.5, 0.5, 0.1], device=mention_logits[0].device)
        mention_loss_fn = FocalLoss(alpha=alpha_mention, gamma=1.0)
    else:
        weights = torch.tensor([0.5, 0.5, 0.1], device=mention_logits[0].device)
    ...
    if use_focal_for_triplet:
        ...
        # weights = torch.tensor([1.0, 1.0, 1.0, 0.1], device=mention_logits[0].device)
...
# def compute_loss(mention_logits, mention_targets, triplet_logits, triplet_targets, use_focal=True):
#     loss = 0

#     if use_focal:
#         alpha = torch.tensor([0.5, 0.5, 0.1], device=mention_logits[0].device)
#         mention_loss_fn = FocalLoss(alpha=alpha, gamma=2.0)
#     else:
#         weights = torch.tensor([0.5, 0.5, 0.1], device=mention_logits[0].device)
#         mention_loss_fn = nn.CrossEntropyLoss(weight=weights)
...
#     for logits, targets in zip(mention_logits, mention_targets):
#         if logits.numel() > 0:
#             loss += mention_loss_fn(logits, targets)

#     if triplet_logits.numel() > 0:
#         triplet_loss_fn = nn.CrossEntropyLoss()
#         loss += triplet_loss_fn(triplet_logits, triplet_targets)

#     return loss


# Loss function for training
# def compute_loss(mention_logits, mention_labels, triplet_logits, triplet_labels):
#     mention_loss_fn = nn.CrossEntropyLoss()
#     triplet_loss_fn = nn.CrossEntropyLoss()

#     # Flatten mention loss computation
#     mention_loss = 0.0
#     total_spans = 0
#     for logit_tensor, label_tensor in zip(mention_logits, mention_labels):
#         if logit_tensor.numel() == 0:
#             continue
#         mention_loss += mention_loss_fn(logit_tensor, label_tensor)
#         total_spans += 1
#     mention_loss = mention_loss / max(total_spans, 1)
...
#     return mention_loss + triplet_loss
